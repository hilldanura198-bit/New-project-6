...
final artworkSearchProvider =
    StateNotifierProvider<ArtworkSearchNotifier, ArtworkSearchState>(
      (ref) =>
          ArtworkSearchNotifier(ref.read(artworkSearchRepositoryProvider))
            ..start(),
    );
...
class ArtworkSearchNotifier extends StateNotifier<ArtworkSearchState> {
  ...
  void clearFilters() {
    state = _applyFilters(
      state.copyWith(query: '', clearCategory: true, clearArtist: true),
    );
  }
  ...
  ArtworkSearchState _applyFilters(ArtworkSearchState base) {
    ...
    final filtered = base.allArtworks.where((artwork) {
      ...
      final artistName = (artwork['artist_name'] ?? '')
          .toString()
          .toLowerCase();
      ...
      final categoryMatch =
          category == null || category.isEmpty || categoryName == category;
      final artistMatch =
          artist == null || artist.isEmpty || artistName == artist;
